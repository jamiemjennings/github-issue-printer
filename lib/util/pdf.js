...
module.exports = {
  createPdf: function (issues, { renderPointsLabels }) {
    ...
    issues.forEach((issue) => {
      ...
      let pointsLabel = issue.pointsLabel
      ...
      if (renderPointsLabels && pointsLabel) {
        let center = { x: borderEndX - 10, y: borderStartY + 45 }
        pdfDoc.circle(center.x, center.y, 40).fillOpacity(1).fill(`#${pointsLabel.color}`)
        pdfDoc.font('Helvetica-Bold').fillColor('white').fontSize(36)
        let textWidth = pdfDoc.widthOfString(pointsLabel.name)
        pdfDoc.text(pointsLabel.name, center.x - Math.floor(textWidth / 2), borderStartY + 30)

        // reset the text starting point
        pdfDoc.text('', 50, 50)
      }
