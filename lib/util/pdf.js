...
module.exports = {
  createPdf: function (issues, { renderSizeLabels }) {
    ...
    issues.forEach((issue) => {
      ...
      if (renderSizeLabels && sizeLabel) {
        let center = { x: borderEndX  - 10, y: borderStartY + 45 }
        pdfDoc.circle(center.x, center.y, 40).fillOpacity(1).fill(`#${sizeLabel.color}`)
        pdfDoc.font('Helvetica-Bold').fillColor('white').fontSize(36)
        let textWidth = pdfDoc.widthOfString(sizeLabel.name)
        pdfDoc.text(sizeLabel.name, center.x - Math.floor(textWidth/2), borderStartY + 30)
